CREATE TABLE IF NOT EXISTS erp_issue_cost_center (
  ...
  movement_type VARCHAR(5) NOT NULL DEFAULT '201',
  ...
  INDEX idx_icc_posting_date (posting_date),
  INDEX idx_icc_cost_center (cost_center_id,cost_center_code),
  INDEX idx_icc_status (status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS erp_issue_cost_center_detail (
  id BIGINT AUTO_INCREMENT PRIMARY KEY,
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_iccd_issue (issue_id),
  INDEX idx_iccd_material (material_code),
  CONSTRAINT fk_iccd_issue FOREIGN KEY (issue_id) REFERENCES erp_issue_cost_center(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS erp_issue_cost_center_trace (
  id BIGINT AUTO_INCREMENT PRIMARY KEY,
  issue_id BIGINT NOT NULL,
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_icct_issue (issue_id),
  INDEX idx_icct_detail (issue_detail_id),
  INDEX idx_icct_layer (stock_layer_id),
  INDEX idx_icct_customs (no_aju,no_dokpab),
  INDEX idx_icct_lot (lot_no),
  CONSTRAINT fk_icct_issue FOREIGN KEY (issue_id) REFERENCES erp_issue_cost_center(id) ON DELETE CASCADE,
  CONSTRAINT fk_icct_detail FOREIGN KEY (issue_detail_id) REFERENCES erp_issue_cost_center_detail(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS erp_issue_cost_center_history (
  id BIGINT AUTO_INCREMENT PRIMARY KEY,
  issue_id BIGINT NOT NULL,
  ...
  INDEX idx_icch_issue (issue_id),
  CONSTRAINT fk_icch_issue FOREIGN KEY (issue_id) REFERENCES erp_issue_cost_center(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

INSERT INTO rekening (no_rek,induk,level,nama_rek,kat_coa,jenis)
SELECT '62199','620',3,'Beban Pemakaian Material Cost Center',18,6
WHERE NOT EXISTS (SELECT 1 FROM rekening WHERE no_rek='62199');
...
SELECT 'issue_to_cost_center','Issue to Cost Center','issue-to-cost-center','erp_issue_cost_center','fa-building-o',2,@gi_parent,'Goods Issue','Y','Y','page'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE url='issue-to-cost-center');

UPDATE sys_menu
SET nav_act='issue_to_cost_center',
    page_name='Issue to Cost Center',
    url='issue-to-cost-center',
    main_table='erp_issue_cost_center',
    icon='fa-building-o',
    urutan_menu=2,
    parent=@gi_parent,
...
WHERE m.url='issue-to-cost-center'
  AND r.id IS NULL;
