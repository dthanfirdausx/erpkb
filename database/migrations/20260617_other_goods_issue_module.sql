CREATE TABLE IF NOT EXISTS erp_other_goods_issue (
  id BIGINT AUTO_INCREMENT PRIMARY KEY,
  issue_no VARCHAR(50) NOT NULL UNIQUE,
  document_date DATE NOT NULL,
  posting_date DATE NOT NULL,
  movement_type VARCHAR(5) NOT NULL DEFAULT '291',
  ...
  INDEX idx_ogi_posting_date (posting_date),
  INDEX idx_ogi_reason (reason_code),
  INDEX idx_ogi_category (issue_category),
  INDEX idx_ogi_recipient (recipient_type,recipient_name),
  INDEX idx_ogi_status (status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS erp_other_goods_issue_detail (
  id BIGINT AUTO_INCREMENT PRIMARY KEY,
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_ogid_issue (issue_id),
  INDEX idx_ogid_material (material_code),
  CONSTRAINT fk_ogid_issue FOREIGN KEY (issue_id) REFERENCES erp_other_goods_issue(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS erp_other_goods_issue_trace (
  id BIGINT AUTO_INCREMENT PRIMARY KEY,
  issue_id BIGINT NOT NULL,
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_ogit_issue (issue_id),
  INDEX idx_ogit_detail (issue_detail_id),
  INDEX idx_ogit_layer (stock_layer_id),
  INDEX idx_ogit_customs (no_aju,no_dokpab),
  INDEX idx_ogit_lot (lot_no),
  CONSTRAINT fk_ogit_issue FOREIGN KEY (issue_id) REFERENCES erp_other_goods_issue(id) ON DELETE CASCADE,
  CONSTRAINT fk_ogit_detail FOREIGN KEY (issue_detail_id) REFERENCES erp_other_goods_issue_detail(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS erp_other_goods_issue_history (
  id BIGINT AUTO_INCREMENT PRIMARY KEY,
  issue_id BIGINT NOT NULL,
  ...
  INDEX idx_ogih_issue (issue_id),
  CONSTRAINT fk_ogih_issue FOREIGN KEY (issue_id) REFERENCES erp_other_goods_issue(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

INSERT INTO rekening (no_rek,induk,level,nama_rek,kat_coa,jenis)
SELECT '62499','620',3,'Beban Other Goods Issue Material',18,6
WHERE NOT EXISTS (SELECT 1 FROM rekening WHERE no_rek='62499');
...
SELECT 'other_goods_issue','Other Goods Issue','other-goods-issue','erp_other_goods_issue','fa-sign-out',8,@gi_parent,'Goods Issue','Y','Y','page'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE url='other-goods-issue');

UPDATE sys_menu
SET nav_act='other_goods_issue',
    page_name='Other Goods Issue',
    url='other-goods-issue',
    main_table='erp_other_goods_issue',
    icon='fa-sign-out',
    urutan_menu=8,
    parent=@gi_parent,
