CREATE TABLE IF NOT EXISTS erp_asset_master (
  ...
  INDEX idx_asset_cost_center (cost_center_id,cost_center_code)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

INSERT INTO erp_asset_master (asset_code,asset_name,asset_class,status)
SELECT 'AST-DEFAULT','Default Asset Assignment','GENERAL','Aktif'
WHERE NOT EXISTS (SELECT 1 FROM erp_asset_master WHERE asset_code='AST-DEFAULT');

CREATE TABLE IF NOT EXISTS erp_issue_asset (
  ...
  movement_type VARCHAR(5) NOT NULL DEFAULT '241',
  ...
  INDEX idx_iasset_posting_date (posting_date),
  INDEX idx_iasset_asset (asset_id,asset_code),
  INDEX idx_iasset_status (status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS erp_issue_asset_detail (
  id BIGINT AUTO_INCREMENT PRIMARY KEY,
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_iassetd_issue (issue_id),
  INDEX idx_iassetd_material (material_code),
  CONSTRAINT fk_iassetd_issue FOREIGN KEY (issue_id) REFERENCES erp_issue_asset(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS erp_issue_asset_trace (
  id BIGINT AUTO_INCREMENT PRIMARY KEY,
  issue_id BIGINT NOT NULL,
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_iassett_issue (issue_id),
  INDEX idx_iassett_detail (issue_detail_id),
  INDEX idx_iassett_layer (stock_layer_id),
  INDEX idx_iassett_customs (no_aju,no_dokpab),
  INDEX idx_iassett_lot (lot_no),
  CONSTRAINT fk_iassett_issue FOREIGN KEY (issue_id) REFERENCES erp_issue_asset(id) ON DELETE CASCADE,
  CONSTRAINT fk_iassett_detail FOREIGN KEY (issue_detail_id) REFERENCES erp_issue_asset_detail(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS erp_issue_asset_history (
  id BIGINT AUTO_INCREMENT PRIMARY KEY,
  issue_id BIGINT NOT NULL,
  ...
  INDEX idx_iasseth_issue (issue_id),
  CONSTRAINT fk_iasseth_issue FOREIGN KEY (issue_id) REFERENCES erp_issue_asset(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

INSERT INTO rekening (no_rek,induk,level,nama_rek,kat_coa,jenis)
SELECT '15199','150',3,'Asset Acquisition Clearing',12,1
WHERE NOT EXISTS (SELECT 1 FROM rekening WHERE no_rek='15199');
...
SELECT 'issue_to_asset','Issue to Asset','issue-to-asset','erp_issue_asset','fa-cubes',3,@gi_parent,'Goods Issue','Y','Y','page'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE url='issue-to-asset');

UPDATE sys_menu
SET nav_act='issue_to_asset',
    page_name='Issue to Asset',
    url='issue-to-asset',
    main_table='erp_issue_asset',
    icon='fa-cubes',
    urutan_menu=3,
    parent=@gi_parent,
