UPDATE sys_menu
SET nav_act='batch_lot_traceability',
    page_name='Batch / Lot Traceability',
    url='batch-lot-traceability',
    main_table='stock_layer',
    icon='fa-random',
    dt_table='Y',
    tampil='Y',
    type_menu='page'
WHERE url='batch-lot-traceability' OR page_name='Batch / Lot Traceability';

INSERT INTO sys_menu (nav_act,page_name,url,main_table,icon,urutan_menu,parent,parent_name,dt_table,tampil,type_menu)
SELECT 'batch_lot_traceability','Batch / Lot Traceability','batch-lot-traceability','stock_layer','fa-random',4,572,'Inventory Management','Y','Y','page'
FROM DUAL
...
WHERE m.url='batch-lot-traceability' AND r.id IS NULL;
