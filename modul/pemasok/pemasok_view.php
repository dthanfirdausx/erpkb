...
?>
 ...
 <div class="alert alert-warning fade in error_data_delete" style="display:none">
          ...
          <i class="icon fa fa-warning"></i> <span class="isi_warning_delete"></span>
                        ...
                        <table id="dtb_pemasok" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                  <th>No</th>
    ...
    <div class="modal" id="modal_pemasok" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"> <div class="modal-dialog modal-lg"> <div class="modal-content"><div class="modal-header"> <button type="button" class="close" data-dismiss="modal" aria-label="<?=erp_attr('common_close','Close');?>"><span aria-hidden="true">×</span></button> <h4 class="modal-title"><?=erp_h('add_button','Add New');?> <?=erp_h('master_vendor_master','Vendor Master');?></h4> </div> <div class="modal-body" id="isi_pemasok"> </div> </div><!-- /.modal-content --> </div><!-- /.modal-dialog --> </div>
      ...
      $("#add_pemasok").click(function() {
          $.ajax({
              url : "<?=base_admin();?>modul/pemasok/pemasok_add.php",
              type : "GET",
              success: function(data) {
                  $("#isi_pemasok").html(data);
              }
          });

      $('#modal_pemasok').modal({ keyboard: false,backdrop:'static',show:true });
    ...
    $(".table").on('click','.edit_data',function(event) {
        ...
        $.ajax({
            url : "<?=base_admin();?>modul/pemasok/pemasok_edit.php",
            type : "post",
            data : {id_data:id},
            success: function(data) {
                $("#isi_pemasok").html(data);
    ...
    $('#modal_pemasok').modal({ keyboard: false,backdrop:'static' });
      ...
      var dtb_pemasok = $("#dtb_pemasok").DataTable({
           ...
           'columnDefs': [ {
            ...
            'ajax':{
              url :'<?=base_admin();?>modul/pemasok/pemasok_data.php',
  ...
  $('#dtb_pemasok').on('draw.dt', function() {
          init_selected()
      });
  ...
  $(document).on('click', '#dtb_pemasok tbody tr td', function(event) {
  ...
  function check_selected() {
      var table_select = $('#dtb_pemasok tbody tr.selected');
  ...
  function select_deselect(type) {
      if (type == 'select') {
          $('#dtb_pemasok tbody tr').addClass('DTTT_selected selected')
      } else {
          $('#dtb_pemasok tbody tr').removeClass('DTTT_selected selected')
      }
      init_selected()
  }
...
/* Add a click handler for the delete row */
  $('#bulk_delete').click( function() {
    var anSelected = fnGetSelected( dtb_pemasok );
    ...
    $('#ucing').modal({ keyboard: false }).one('click', '#delete', function (e) {
        ...
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: '<?=base_admin();?>modul/pemasok/pemasok_action.php?act=del_massal',
