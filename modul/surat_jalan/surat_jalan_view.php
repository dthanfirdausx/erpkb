...
var dtb_surat_jalan = $("#dtb_surat_jalan").DataTable({
    "fnCreatedRow": function(nRow, aData, iDataIndex) {
        ...
        var statusText = aData[8]; // Kolom status (index 7)
    //    alert(statusText);
        var badge_class = '';
...
// 🔍 DETAIL
buttons += '<button onclick="show_detail(\'' + aData[indek] + '\')" class="btn btn-success" title="Detail">' +
           '<i class="fa fa-eye"></i></button>';

// 🖨 PRINT
buttons += '<a href="<?=base_url();?>modul/surat_jalan/surat_jalan_print.php?id=' + aData[indek] + '" target="_blank" class="btn btn-primary" title="Print">' +
           '<i class="fa fa-print"></i></a>';

// ✏️ EDIT (hanya draft)
if (statusText.toLowerCase() === 'draft') {
    buttons += '<a href="<?=base_index();?>surat-jalan/edit/' + aData[indek] + '" class="btn btn-warning" title="Edit">' +
               '<i class="fa fa-pencil"></i></a>';
}

// 🗑 DELETE (hanya draft)
if (statusText.toLowerCase() === 'draft') {
    buttons += '<button onclick="delete_surat_jalan(\'' + aData[indek] + '\')" class="btn btn-danger" title="Hapus">' +
