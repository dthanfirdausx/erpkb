...
function cst_h($value) {
  return htmlspecialchars((string)$value, ENT_QUOTES, 'UTF-8');
}

function cst_input($key, $default = '') {
  if (isset($_POST[$key])) return trim((string)$_POST[$key]);
  if (isset($_GET[$key])) return trim((string)$_GET[$key]);
  return $default;
}

function cst_valid_date($date, $default) {
  $date = trim((string)$date);
  return preg_match('/^\d{4}-\d{2}-\d{2}$/', $date) ? $date : $default;
}

function cst_stock_type_label($stockType) {
...
function cst_filter_sql($input, &$params) {
  $where = " WHERE 1=1 ";
  ...
  if (!empty($input['tgl_awal'])) { $where .= " AND COALESCE(sl.tgl_masuk,DATE(sl.created_at))>=? "; $params[] = cst_valid_date($input['tgl_awal'], date('Y-m-01')); }
  if (!empty($input['tgl_akhir'])) { $where .= " AND COALESCE(sl.tgl_masuk,DATE(sl.created_at))<=? "; $params[] = cst_valid_date($input['tgl_akhir'], date('Y-m-d')); }
...
function cst_load_layers($db, $input) {
  $params = array();
  $where = cst_filter_sql($input, $params);
  return $db->query(
...
function cst_group_layers($layers) {
  $groups = array();
