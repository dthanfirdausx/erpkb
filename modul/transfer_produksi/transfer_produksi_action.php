...
// ORDER BY d.no, dt.no_aju";
    $q = $db->query("
SELECT d.no, dt.no_dokpab, dt.no_aju, b.nm_barang, t.no_transfer as no_spb, t.tgl_transfer as tgl_spb, b.kd_barang as kode, MAX(p.jenis_dokpab) as jenis_dokpab, 
IFNULL(MAX(rd.jumlah),0) as qtyro, abs(dt.qty) as jumlah, b.satuan, t.ket FROM transfer t JOIN transfer_detail d ON d.id_transfer = t.id_transfer JOIN barang b ON b.id = d.id_barang 
JOIN detail_transaksi dt ON (dt.no_ref = t.no_transfer AND dt.kd_barang = b.kd_barang AND dt.posisi = 'GUDANG' AND dt.is_reversal='0' and dt.no_urut=d.no) 
LEFT JOIN pemasukan p ON p.no_aju = dt.no_aju LEFT JOIN ro ON ro.no_ro = t.no_ro LEFT JOIN ro_detail rd ON (rd.no_ro = ro.no_ro AND rd.kode = b.kd_barang and rd.row_no=dt.no_urut) WHERE t.no_transfer = '$no_spb'
GROUP BY d.no, dt.no_aju, dt.no_dokpab ORDER BY d.no
");
    $no = 0;
    $noo=1;
    foreach ($q as $k) {     
...
foreach ($_POST['kode_input'] as $key => $value) {    
    ...
    $cek = $db->query("
        ...
        FROM stock_layer
        WHERE kode = '".$kode_barang."' AND qty_sisa > 0 and lokasi='GUDANG' 
    ")->fetch();
    ...
    $q_layer = $db->query("
        ...
        AND qty_sisa > 0 and lokasi='GUDANG'
