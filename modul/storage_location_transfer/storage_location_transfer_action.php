...
function slt_json($status, $message = '', $extra = array()) {
...
function slt_h($value) { return htmlspecialchars((string)$value, ENT_QUOTES, 'UTF-8'); }
function slt_num($value, $dec = 5) { return number_format((float)$value, $dec, ',', '.'); }
function slt_clean_qty($value) { return (float)str_replace(',', '.', trim((string)$value)); }
function slt_valid_date($date) {
  $dt = DateTime::createFromFormat('Y-m-d', $date);
  return $dt && $dt->format('Y-m-d') === $date;
}
function slt_next_number($postingDate) {
  global $db;
  $prefix = 'SLT'.date('Ym', strtotime($postingDate ?: date('Y-m-d')));
  $row = $db->fetch("SELECT transfer_no FROM erp_storage_location_transfer WHERE transfer_no LIKE ? ORDER BY transfer_no DESC LIMIT 1", array($prefix.'%'));
  $next = 1;
  if ($row && preg_match('/(\d{5})$/', $row->transfer_no, $m)) $next = intval($m[1]) + 1;
  return $prefix.sprintf('%05d', $next);
}
function slt_layer_where($materialCode, $storageLocationId, $storageBinId) {
...
function slt_layer_price($layer) {
...
function slt_fetch_layers($materialCode, $storageLocationId, $storageBinId, $forUpdate = false) {
  global $db;
  list($where, $params) = slt_layer_where($materialCode, $storageLocationId, $storageBinId);
...
function slt_location($storageLocationId, $storageBinId = 0) {
  global $db;
  return $db->fetch(
...
function slt_insert_material_doc($data) {
  global $db;
  if (!$db->insert('detail_transaksi', $data)) return false;
  return $db->last_insert_id();
}
function slt_render_detail($id) {
  global $db;
  $h = $db->fetch(
  ...
  if (!$h) { echo '<div class="alert alert-warning">Storage Location Transfer tidak ditemukan.</div>'; return; }
  $items = $db->query("SELECT * FROM erp_storage_location_transfer_detail WHERE transfer_id=? ORDER BY line_no,id", array($id));
  $history = $db->query("SELECT * FROM erp_storage_location_transfer_history WHERE transfer_id=? ORDER BY changed_at DESC,id DESC", array($id));
  ?>
  <div class="row">
    <div class="col-md-8"><h3 style="margin-top:0;font-weight:700"><?=slt_h($h->transfer_no);?> <small>MvT <?=slt_h($h->movement_type);?></small></h3><p class="text-muted"><?=slt_h($h->reason_code.' - '.$h->reason_text);?></p></div>
    <div class="col-md-4 text-right"><span class="label label-<?=($h->status==='POSTED'?'success':'danger');?>"><?=slt_h($h->status);?></span></div>
  </div>
  <div class="row">
    <div class="col-sm-3"><strong><?=wh_h(wh_t('warehouse_posting_date', 'Posting Date'));?></strong><br><?=slt_h($h->posting_date);?></div>
    <div class="col-sm-3"><strong>Source</strong><br><?=slt_h(trim($h->source_plant_code.' / '.$h->source_storage_code.' / '.$h->source_bin_code, ' /'));?></div>
    <div class="col-sm-3"><strong>Destination</strong><br><?=slt_h(trim($h->destination_plant_code.' / '.$h->destination_storage_code.' / '.$h->destination_bin_code, ' /'));?></div>
    <div class="col-sm-3"><strong>Created By</strong><br><?=slt_h($h->created_by);?></div>
  </div>
  <hr>
  <?php foreach ($items as $item) {
    $traces = $db->query(
      "SELECT t.*,ssl.storage_code AS source_storage_code,ds.storage_code AS destination_storage_code,sb.bin_code AS source_bin_code,dbin.bin_code AS destination_bin_code
       FROM erp_storage_location_transfer_trace t
  ...
  ?>
    <h4><?=slt_h($item->line_no.'. '.$item->material_code.' - '.$item->material_name);?> <small>Qty <?=slt_num($item->qty).' '.slt_h($item->uom);?> | Amount <?=number_format((float)$item->amount,2,',','.');?></small></h4>
      ...
      <table class="table table-bordered table-condensed">
        ...
        <?php foreach ($traces as $t) { ?>
          <tr>
            ...
            <td class="text-right"><?=slt_num($t->qty);?></td>
            <td class="text-right"><?=number_format((float)$t->price,5,',','.');?></td>
            <td><?=slt_h(trim($t->source_storage_code.' / '.$t->source_bin_code, ' /'));?></td>
            <td><?=slt_h(trim($t->destination_storage_code.' / '.$t->destination_bin_code, ' /'));?></td>
            <td><?=slt_h($t->no_bpb);?></td>
            <td><?=slt_h($t->no_aju);?></td>
            <td><?=slt_h(trim($t->jenis_dokpab.' '.$t->no_dokpab));?></td>
  ...
  <ul class="list-unstyled">
    <?php foreach ($history as $row) { ?><li><strong><?=slt_h(($row->status_lama ?: '-').' -> '.$row->status_baru);?></strong> <span class="text-muted"><?=slt_h($row->changed_by.' @ '.$row->changed_at);?></span><br><?=slt_h($row->remarks);?></li><?php } ?>
  </ul>
  <?php
}
...
switch ($act) {
  case 'material_search':
    ...
    $results = array();
    foreach ($rows as $row) $results[] = array('id'=>$row->kode,'text'=>$row->kode.' - '.$row->nm_barang.' | Stock '.slt_num($row->stock).' '.$row->satuan,'material_name'=>$row->nm_barang,'uom'=>$row->satuan,'stock'=>(float)$row->stock);
  ...
  case 'stock_preview':
    ...
    $layers = slt_fetch_layers($materialCode, $storageLocationId, $storageBinId, false);
    ...
    foreach ($layers as $layer) echo '<tr><td>#'.intval($layer->id).'</td><td class="text-right">'.slt_num($layer->qty_sisa).'</td><td>'.slt_h($layer->stock_type).'</td><td>'.slt_h($layer->no_bpb ?: '-').'<br><small>'.slt_h(trim($layer->jenis_dokpab.' '.$layer->no_dokpab).' / '.$layer->no_aju).'</small></td></tr>';
    echo '</tbody></table>';
    break;
  ...
  case 'post':
    ...
    if (!slt_valid_date($documentDate) || !slt_valid_date($postingDate)) slt_json('error', 'Document Date dan Posting Date wajib valid.');
    if ($sourceStorageLocationId <= 0 || $destinationStorageLocationId <= 0) slt_json('error', 'Source dan Destination Storage Location wajib diisi.');
    if ($sourceStorageLocationId === $destinationStorageLocationId && $sourceStorageBinId === $destinationStorageBinId) slt_json('error', 'Source dan destination tidak boleh sama.');
    if ($reasonCode === '' || $reasonText === '') slt_json('error', 'Reason wajib diisi.');
    $sourceLoc = slt_location($sourceStorageLocationId, $sourceStorageBinId);
    $destLoc = slt_location($destinationStorageLocationId, $destinationStorageBinId);
    if (!$sourceLoc || !$destLoc) slt_json('error', 'Source atau destination storage location tidak valid.');
    if ((int)$sourceLoc->plant_id !== (int)$destLoc->plant_id) slt_json('error', 'Movement 311 hanya untuk transfer storage location dalam plant yang sama.');
    if ($sourceStorageBinId > 0 && empty($sourceLoc->bin_id)) slt_json('error', 'Source storage bin tidak valid.');
    if ($destinationStorageBinId > 0 && empty($destLoc->bin_id)) slt_json('error', 'Destination storage bin tidak valid.');
    if (empty($_POST['material_code']) || !is_array($_POST['material_code'])) slt_json('error', 'Minimal satu item material wajib diisi.');
    $items = array();
    foreach ($_POST['material_code'] as $idx => $code) {
      $code = trim((string)$code);
      $qty = isset($_POST['qty'][$idx]) ? slt_clean_qty($_POST['qty'][$idx]) : 0;
      if ($code !== '' && $qty > 0) $items[] = array('material_code'=>$code,'qty'=>$qty,'remarks'=>isset($_POST['item_remarks'][$idx]) ? trim($_POST['item_remarks'][$idx]) : '');
    }
    if (!$items) slt_json('error', 'Transfer Qty wajib lebih dari nol.');
    $db->query('START TRANSACTION');
    $transferNo = slt_next_number($postingDate);
    if (!$db->insert('erp_storage_location_transfer', array(
    ...
    ))) { $err=$db->getErrorMessage(); $db->query('ROLLBACK'); slt_json('error',$err ?: 'Header transfer gagal disimpan.'); }
    ...
    foreach ($items as $item) {
      ...
      if (!$material) { $db->query('ROLLBACK'); slt_json('error', 'Material '.$item['material_code'].' tidak ditemukan.'); }
      list($layerWhere, $layerParams) = slt_layer_where($item['material_code'], $sourceStorageLocationId, $sourceStorageBinId);
      $available = $db->fetch("SELECT COALESCE(SUM(sl.qty_sisa),0) AS available_qty FROM stock_layer sl ".$layerWhere, $layerParams);
      if (!$available || (float)$available->available_qty + 0.00001 < $item['qty']) { $db->query('ROLLBACK'); slt_json('error', 'Stock tidak cukup untuk '.$item['material_code'].'.'); }
      if (!$db->insert('erp_storage_location_transfer_detail', array('transfer_id'=>$transferId,'line_no'=>$lineNo,'material_code'=>$material->kd_barang,'material_name'=>$material->nm_barang,'qty'=>$item['qty'],'uom'=>$material->satuan,'remarks'=>$item['remarks']))) {
        $err=$db->getErrorMessage(); $db->query('ROLLBACK'); slt_json('error',$err ?: 'Detail transfer gagal disimpan.');
      }
      $detailId = $db->last_insert_id();
      $remaining = $item['qty'];
      $detailAmount = 0;
      $weightedValue = 0;
      $layers = slt_fetch_layers($item['material_code'], $sourceStorageLocationId, $sourceStorageBinId, true);
      foreach ($layers as $layer) {
        if ($remaining <= 0) break;
        $take = min($remaining, (float)$layer->qty_sisa);
        if ($take <= 0) continue;
        $price = slt_layer_price($layer);
        ...
        if (!$ok) { $err=$db->getErrorMessage(); $db->query('ROLLBACK'); slt_json('error',$err ?: 'Source stock layer gagal dikurangi.'); }
        if (!$db->insert('stock_layer', array(
          ...
          'jenis_dokpab'=>$layer->jenis_dokpab,'ref_table'=>'erp_storage_location_transfer_trace','ref_id'=>null,'tgl_masuk'=>$postingDate,'no_bpb'=>$layer->no_bpb
        ))) { $err=$db->getErrorMessage(); $db->query('ROLLBACK'); slt_json('error',$err ?: 'Destination stock layer gagal dibuat.'); }
        $destLayerId = $db->last_insert_id();
        $outDocId = slt_insert_material_doc(array('no_ref'=>$transferNo,'ref_pengganti'=>$referenceNo,'no_aju'=>$layer->no_aju,'no_dokpab'=>$layer->no_dokpab,'id_incoming_detail'=>($layer->ref_table === 'pemasukan_detail') ? $layer->ref_id : null,'move_code'=>'311','posisi'=>'GUDANG','no_urut'=>$lineNo,'qty'=>$take * -1,'kd_barang'=>$material->kd_barang,'lokasi'=>'GUDANG','document_date'=>$documentDate,'posting_date'=>$postingDate,'user'=>$username,'direction'=>'OUT','ref_type'=>'SLT','ref_id'=>$layer->id,'ref_detail_id'=>$detailId,'uom'=>$material->satuan,'price'=>$price,'amount'=>$amount,'reason'=>$reasonCode,'created_by'=>$username,'no_bpb'=>$layer->no_bpb,'plant_id'=>$layer->plant_id,'storage_location_id'=>$layer->storage_location_id,'storage_bin_id'=>$layer->storage_bin_id,'stock_type'=>$layer->stock_type,'destination_storage_location_id'=>$destinationStorageLocationId,'destination_storage_bin_id'=>$destinationStorageBinId ?: null,'destination_stock_type'=>$layer->stock_type,'destination_material_code'=>$material->kd_barang,'remark'=>'Storage Location Transfer 311 OUT '.$transferNo));
        if (!$outDocId) { $err=$db->getErrorMessage(); $db->query('ROLLBACK'); slt_json('error',$err ?: 'Material document OUT gagal disimpan.'); }
        $inDocId = slt_insert_material_doc(array('no_ref'=>$transferNo,'ref_pengganti'=>$referenceNo,'no_aju'=>$layer->no_aju,'no_dokpab'=>$layer->no_dokpab,'id_incoming_detail'=>($layer->ref_table === 'pemasukan_detail') ? $layer->ref_id : null,'move_code'=>'311','posisi'=>'GUDANG','no_urut'=>$lineNo,'qty'=>$take,'kd_barang'=>$material->kd_barang,'lokasi'=>'GUDANG','document_date'=>$documentDate,'posting_date'=>$postingDate,'user'=>$username,'direction'=>'IN','ref_type'=>'SLT','ref_id'=>$destLayerId,'ref_detail_id'=>$outDocId,'uom'=>$material->satuan,'price'=>$price,'amount'=>$amount,'reason'=>$reasonCode,'created_by'=>$username,'no_bpb'=>$layer->no_bpb,'plant_id'=>$destLoc->plant_id,'storage_location_id'=>$destinationStorageLocationId,'storage_bin_id'=>$destinationStorageBinId ?: null,'stock_type'=>$layer->stock_type,'destination_storage_location_id'=>$destinationStorageLocationId,'destination_storage_bin_id'=>$destinationStorageBinId ?: null,'destination_stock_type'=>$layer->stock_type,'destination_material_code'=>$material->kd_barang,'remark'=>'Storage Location Transfer 311 IN '.$transferNo));
        if (!$inDocId) { $err=$db->getErrorMessage(); $db->query('ROLLBACK'); slt_json('error',$err ?: 'Material document IN gagal disimpan.'); }
        if (!$db->insert('erp_storage_location_transfer_trace', array(
        ...
        ))) { $err=$db->getErrorMessage(); $db->query('ROLLBACK'); slt_json('error',$err ?: 'Trace transfer gagal disimpan.'); }
      ...
      if ($remaining > 0.00001) { $db->query('ROLLBACK'); slt_json('error','Stock layer tidak cukup untuk '.$item['material_code'].'.'); }
      $detailPrice = $item['qty'] > 0 ? $weightedValue / $item['qty'] : 0;
      $db->query("UPDATE erp_storage_location_transfer_detail SET price=?,amount=? WHERE id=?", array($detailPrice,$detailAmount,$detailId));
      $lineNo++;
    }
    $db->insert('erp_storage_location_transfer_history', array('transfer_id'=>$transferId,'status_baru'=>'POSTED','remarks'=>'Storage Location Transfer 311 posted','changed_by'=>$username));
    if (function_exists('simpan_log')) simpan_log('User '.$username.' posting Storage Location Transfer '.$transferNo.' pada '.date('Y-m-d H:i:s'), $username);
    $db->query('COMMIT');
    slt_json('good','',array('transfer_no'=>$transferNo));
    break;

  case 'detail':
    slt_render_detail(isset($_POST['id']) ? (int)$_POST['id'] : 0);
    break;

  case 'reversal':
    $id = isset($_POST['id']) ? (int)$_POST['id'] : 0;
    $reason = isset($_POST['reason']) ? trim($_POST['reason']) : '';
    if ($id <= 0 || $reason === '') slt_json('error','Dokumen dan alasan reversal wajib diisi.');
    $h = $db->fetch("SELECT * FROM erp_storage_location_transfer WHERE id=? LIMIT 1", array($id));
    if (!$h) slt_json('error','Transfer tidak ditemukan.');
    if ($h->status === 'REVERSED') slt_json('error','Transfer sudah reversal.');
    $traces = $db->query("SELECT t.*,d.material_code,d.material_name,d.uom,d.line_no FROM erp_storage_location_transfer_trace t JOIN erp_storage_location_transfer_detail d ON d.id=t.transfer_detail_id WHERE t.transfer_id=? ORDER BY t.id", array($id));
    $db->query('START TRANSACTION');
    ...
    foreach ($traces as $t) {
      ...
      if (!$dest || (float)$dest->qty_sisa + 0.00001 < $qty) { $db->query('ROLLBACK'); slt_json('error','Destination stock layer #'.$t->destination_stock_layer_id.' tidak cukup untuk reversal.'); }
  ...
  case 'excel':
    ...
    $where = " WHERE 1=1 ";
    $params = array();
    if (slt_valid_date($tglAwal) && slt_valid_date($tglAkhir)) { $where .= " AND h.posting_date BETWEEN ? AND ? "; $params[]=$tglAwal; $params[]=$tglAkhir; }
    ...
    $rows = $db->query(
      "SELECT h.transfer_no,h.posting_date,h.document_date,h.reason_code,h.reason_text,h.status,
       ...
       FROM erp_storage_location_transfer h
       JOIN erp_storage_location_transfer_detail d ON d.transfer_id=h.id
       LEFT JOIN erp_storage_location_transfer_trace t ON t.transfer_detail_id=d.id
    ...
    $sheet->setTitle(erp_export_sheet_title('SLoc Transfer'));
    $sheet->mergeCells('A1:V1'); $sheet->mergeCells('A2:V2');
    $sheet->setCellValue('A1', namaPT); $sheet->setCellValue('A2', 'STORAGE LOCATION TRANSFER');
    ...
    header('Content-Disposition: attachment; filename="storage_location_transfer_'.$tglAwal.'_sd_'.$tglAkhir.'.xlsx"');
    header('Content-Length: '.$size);
    readfile($tmp); @unlink($tmp); exit;

  default:
    slt_json('error','Action tidak dikenal.');
}
?>
