...
function gir_base_from_sql() {
  $material = gir_material_expr();
  $sloc = gir_sloc_expr();
  $bin = gir_bin_expr();
...
function gir_filter_sql($input, &$params) {
  $where = " WHERE ".gir_material_expr()." IS NOT NULL ";
  $where .= " AND dt.move_code IN ('122','201','221','241','251','261','291','551','555','601','633','641','701','702') ";
  $from = gir_valid_date(isset($input['tgl_awal']) ? $input['tgl_awal'] : '', date('Y-m-01'));
  $to = gir_valid_date(isset($input['tgl_akhir']) ? $input['tgl_akhir'] : '', date('Y-m-d'));
  $where .= " AND dt.posting_date BETWEEN ? AND ? ";
  $params[] = $from.' 00:00:00';
  $params[] = $to.' 23:59:59';

  if (!empty($input['material_code'])) { $where .= " AND ".gir_material_expr()."=? "; $params[] = $input['material_code']; }
  if (!empty($input['plant_id'])) { $where .= " AND COALESCE(dt.plant_id,es.plant_id,slocfb.plant_id)=? "; $params[] = (int)$input['plant_id']; }
  if (!empty($input['storage_location_id'])) { $where .= " AND ".gir_sloc_expr()."=? "; $params[] = (int)$input['storage_location_id']; }
  if (!empty($input['storage_bin_id'])) { $where .= " AND ".gir_bin_expr()."=? "; $params[] = (int)$input['storage_bin_id']; }
  if (!empty($input['stock_type'])) { $where .= " AND ".gir_stock_type_expr()."=? "; $params[] = $input['stock_type']; }
  ...
  if (!empty($input['direction'])) {
    if ($input['direction'] === 'IN') $where .= " AND ".gir_direction_expr()."='IN' ";
    if ($input['direction'] === 'OUT') $where .= " AND ".gir_direction_expr()."='OUT' ";
  }
...
function gir_input_array() {
  return array(
    'tgl_awal' => gir_input('tgl_awal', date('Y-m-01')),
    'tgl_akhir' => gir_input('tgl_akhir', date('Y-m-d')),
    'material_code' => gir_input('material_code'),
    'plant_id' => gir_input('plant_id'),
    'storage_location_id' => gir_input('storage_location_id'),
    'storage_bin_id' => gir_input('storage_bin_id'),
    'stock_type' => gir_input('stock_type'),
    'move_code' => gir_input('move_code'),
    'ref_type' => gir_input('ref_type'),
    'direction' => gir_input('direction'),
    'user' => gir_input('user'),
    'keyword' => gir_input('keyword')
  );
}

function gir_allowed_move_sql() {
  return " move_code IN ('122','201','221','241','251','261','291','551','555','601','633','641','701','702') ";
}

function gir_load_rows($db, $input, $limit = 0, $offset = 0) {
  $params = array();
  $where = gir_filter_sql($input, $params);
  $sql = "SELECT dt.id_detail,dt.no_ref,dt.ref_pengganti,dt.no_bpb,dt.no_aju,dt.no_dokpab,
                 ...
                 ".gir_material_expr()." AS material_code,
                 b.nm_barang,b.satuan,
                 COALESCE(dt.plant_id,es.plant_id,slocfb.plant_id) AS plant_id,
                 ep.plant_code,ep.plant_name,
                 ".gir_sloc_expr()." AS storage_location_id,
                 es.storage_code,es.storage_name,
                 ".gir_bin_expr()." AS storage_bin_id,
                 eb.bin_code,eb.bin_name,
                 ".gir_stock_type_expr()." AS stock_type_label,
                 ".gir_direction_expr()." AS movement_direction,
                 ".gir_signed_qty_expr()." AS signed_qty,
...
function gir_count_rows($db, $input) {
  $params = array();
  $where = gir_filter_sql($input, $params);
  $row = $db->fetch("SELECT COUNT(*) AS total ".gir_base_from_sql()." $where", $params);
  return $row ? (int)$row->total : 0;
}

function gir_summary($db, $input) {
  $params = array();
  $where = gir_filter_sql($input, $params);
  return $db->fetch("SELECT COUNT(*) AS total_lines,
                            ...
                            COALESCE(SUM(CASE WHEN ".gir_direction_expr()."='IN' THEN ABS(COALESCE(dt.qty,0)) ELSE 0 END),0) AS qty_in,
                            COALESCE(SUM(CASE WHEN ".gir_direction_expr()."='OUT' THEN ABS(COALESCE(dt.qty,0)) ELSE 0 END),0) AS qty_out,
                            COALESCE(SUM(COALESCE(dt.amount,0)),0) AS total_amount
                     ".gir_base_from_sql()."
                     $where", $params);
}

function gir_issue_type_label($moveCode, $refType) {
  if ($moveCode === '122') return 'Return to Vendor';
  if ($moveCode === '201') return 'Issue to Cost Center';
  if ($moveCode === '221') return 'Issue to Project';
  if ($moveCode === '241') return 'Issue to Asset';
  if ($moveCode === '251') return 'Issue to Sales';
  if ($moveCode === '261') return 'Issue to Production';
  if ($moveCode === '291') return 'Other Goods Issue';
  if ($moveCode === '551') return 'Scrap Issue';
  if ($moveCode === '555') return 'Sample Issue';
  if ($moveCode === '601') return 'Goods Issue for Delivery';
  if ($moveCode === '633') return 'Sample Consumption';
  if ($moveCode === '641') return 'GI for Stock Transport';
  if ($moveCode === '701') return 'Physical Inventory Gain';
  if ($moveCode === '702') return 'Physical Inventory Loss';
  return $refType ?: 'Goods Issue';
}

function gir_stock_type_label($stockType) {
...
function gir_movement_label($moveCode, $refType, $direction) {
  $labels = array(
    ...
    '633'=>'Sample Consumption',
    '641'=>'GI for Stock Transport',
  ...
  return $direction === 'OUT' ? 'Goods Issue' : 'Goods Issue';
}
?>
