...
function sbtd_h($value) { return htmlspecialchars((string)$value, ENT_QUOTES, 'UTF-8'); }
function sbtd_status($status) {
  ...
  return '<span class="label label-default">'.sbtd_h($status).'</span>';
}
...
if (!empty($_POST['keyword'])) {
  ...
  $where .= " AND (
    ...
    OR EXISTS (SELECT 1 FROM erp_storage_bin_transfer_detail d WHERE d.transfer_id=h.id AND (d.material_code LIKE ? OR d.material_name LIKE ?))
    OR EXISTS (SELECT 1 FROM erp_storage_bin_transfer_trace t WHERE t.transfer_id=h.id AND (t.no_aju LIKE ? OR t.no_dokpab LIKE ? OR t.no_bpb LIKE ?))
  ) ";
...
$query = $datatable->get_custom(
  "SELECT h.*,
   ...
   LEFT JOIN (
     ...
     FROM erp_storage_bin_transfer_trace
     GROUP BY transfer_id
...
foreach ($query as $row) {
  $actions = '<div class="sbt-action-buttons">';
  $actions .= '<button type="button" class="btn btn-info btn-xs btn-detail-sbt" data-id="'.intval($row->id).'" title="'.wh_h(wh_t('common_detail', 'Detail')).'"><i class="fa fa-eye"></i> <span class="badge">'.intval($row->item_count).'</span></button>';
  if ($row->status === 'POSTED') {
    $actions .= ' <button type="button" class="btn btn-warning btn-xs btn-reversal-sbt" data-id="'.intval($row->id).'" data-no="'.sbtd_h($row->transfer_no).'" title="'.wh_h(wh_t('warehouse_reversal', 'Reversal')).' 312"><i class="fa fa-undo"></i></button>';
  }
  $actions .= '</div>';
  $doc = '<strong>'.sbtd_h($row->transfer_no).'</strong><br><small>MvT '.sbtd_h($row->movement_type).'</small>';
  $source = '<strong>'.sbtd_h(trim($row->source_plant_code.' / '.$row->source_storage_code, ' /')).'</strong><br><small>'.sbtd_h(trim($row->source_storage_name.' / '.$row->source_bin_code, ' /')).'</small>';
  $dest = '<strong>'.sbtd_h(trim($row->destination_plant_code.' / '.$row->destination_storage_code, ' /')).'</strong><br><small>'.sbtd_h(trim($row->destination_storage_name.' / '.$row->destination_bin_code, ' /')).'</small>';
  $reason = '<strong>'.sbtd_h($row->reason_code ?: '-').'</strong><br><small>'.sbtd_h($row->reason_text ?: '-').'</small>';

  $result = array();
  $result[] = $datatable->number($i);
  $result[] = $actions;
  $result[] = $doc;
  $result[] = sbtd_h($row->posting_date);
  $result[] = $source;
  $result[] = $dest;
  $result[] = $reason;
  ...
  $result[] = sbtd_status($row->status);
  $result[] = sbtd_h($row->created_by);
  $data[] = $result;
  $i++;
}
