...
switch ($_GET["act"]) {
  ...
  case "buat_view":
   ...
   $db->query("drop view v_hitung_mutasi_barang_modal"); 
   $db->query("drop view v_mutasi_barang_modal"); 
   $db->query("create view v_hitung_mutasi_barang_modal as select b.id, b.satuan, b.kd_barang,b.nm_barang,ifnull(p1.jml,0) as p1,ifnull(p2.jml,0) as p2,ifnull(p3.jml,0) as p3,ifnull(p4.jml,0) as p4,ifnull(k.jml,0) as k1,
...
where b.kd_kategori='K03'
");

   $db->query(" create view v_mutasi_barang_modal as
    select  id, `v_hitung_mutasi_barang_modal`.`kd_barang` AS `kd_barang`,`v_hitung_mutasi_barang_modal`.`nm_barang` AS `nm_barang`,`v_hitung_mutasi_barang_modal`.`satuan` AS `satuan`,`v_hitung_mutasi_barang_modal`.`p1` + `v_hitung_mutasi_barang_modal`.`p3` - (`v_hitung_mutasi_barang_modal`.`k2` + `v_hitung_mutasi_barang_modal`.`k1`) AS `saldo_awal`,`v_hitung_mutasi_barang_modal`.`p2` + `v_hitung_mutasi_barang_modal`.`p4` AS `pemasukan`,`v_hitung_mutasi_barang_modal`.`k3` + `v_hitung_mutasi_barang_modal`.`k4` AS `pengeluaran`,`v_hitung_mutasi_barang_modal`.`p1` + `v_hitung_mutasi_barang_modal`.`p3` - (`v_hitung_mutasi_barang_modal`.`k2` + `v_hitung_mutasi_barang_modal`.`k1`) + (`v_hitung_mutasi_barang_modal`.`p2` + `v_hitung_mutasi_barang_modal`.`p4`) - (`v_hitung_mutasi_barang_modal`.`k3` + `v_hitung_mutasi_barang_modal`.`k4`) AS `saldo_akhir`,'0' AS `penyesuaian`,'0' AS `stock_opname`,'0' AS `selisih`,'' AS `ket`,'".$_SESSION['username']."' as userid from `v_hitung_mutasi_barang_modal`");  
  ...
  $data = array(
    ...
    $in = $db->insert("mutasi_barangmodal",$data);
  ...
  case "delete":
    ...
    $db->delete("mutasi_barangmodal","id",$_GET["id"]);
    action_response($db->getErrorMessage());
    break;
   case "del_massal":
    ...
    if(!empty($data_id_array)) {
        foreach($data_id_array as $id) {
          $db->delete("mutasi_barangmodal","id",$id);
         }
    }
    action_response($db->getErrorMessage());
    break;
  case "up":
   ...
   $data = array(
    ...
    $up = $db->update("mutasi_barangmodal",$data,"id",$_POST["id"]);
