      ...
      <div class="col-lg-12">
        <div class="box box-solid box-primary">
          ...
          <span class="isi_warning"></span>
            ...
            <form id="input_lp_barang_jadi" method="post" class="form-horizontal foto_banyak" action="<?=base_admin();?>modul/lp_gabungan/lp_gabungan_action.php?act=in">
              <div class="form-group">
                        <label for="Departemen" class="control-label col-lg-2">Jenis Produksi <span style="color:#FF0000">*</span></label>
                        <div class="col-lg-10"> 
                      <select  id="jenis_produksi" name="jenis_produksi" data-placeholder="Pilih Jenis Produksi ..." class="form-control chzn-select" tabindex="2" required>
                         <option value=""></option>
                ...
                <div class="form-group" id="form_ro" >
                 ...
                 <thead>
                   <tr>
                     ...
                     <th style="width: 300px">Kode /Nama Barang Jadi</th>
                   <!--   <th style="width: 150px">Jenis Dokpab</th> -->
                     <th style="width: 100px">Unit</th>
                    <!--  <th>Qty RO</th> -->
                   <!--   <th>Stock Bahan Baku</th> -->
                     <th>Qty</th>                     
                     <th>Ket</th>
                     <th></th>
                   </tr>
                 ...
                 <tbody id="isi_tabel">
                   <tr id="baris_1">
                     ...
                     <td><input type="text" id="form_kode_1" placeholder="Kode Barang" onclick="cari_kode('1')" class="form-control" name="kode[]"  >
                      ...
                      <i id="error_form_kode_1" style="color: red;display: none">Silahkan Pilih Barang Jadi</i>
                     </td> 
                     ...
                     <td><input type="text" id="form_qty_1" class="form-control" name="qty[]" onkeyup="cek_stok('1',this.value)" required>
                      <i id="error_form_qty_1" style="color: red;display: none">Silahkan input Qty</i>
                     <i id="error_stock_1" style="color: red"></i> </td>
                     <td><input type="text" id="form_ket_1" class="form-control" name="ket[]" ></td>
                     <td><a onclick="add_bahan_baku(1)" class="btn btn-primary"><i class="fa fa-list-ol"></i> Bahan Baku/Scrap</a></td>
                   </tr>
                 </tbody>
               </table>
                 </div>
      ...
      <div class="modal-dialog modal-lg" style="width: 90%">
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title text-center">Detail Bahan Baku/Scrap LP Gabungan</h4>
          </div>
          <div class="modal-body" id="detail_bahan_baku">
...
<script type="text/javascript">
   ...
   function add_bahan_baku(id){
    if ($("#kode_input_"+id).val()=='' && $("#form_qty_"+id).val()=='') {
      $("#error_form_kode_"+id).show();
      $("#error_form_qty_"+id).show();
      $("#form_kode_"+id).focus();
    }else if($("#kode_input_"+id).val()!='' && $("#form_qty_"+id).val()==''){
       // $("#error_form_kode_"+id).show();
      $("#error_form_qty_"+id).show();
      $("#form_qty_"+id).focus();
    }else if($("#kode_input_"+id).val()=='' && $("#form_qty_"+id).val()!=''){
      ...
      $("#error_form_kode_"+id).hide();
      $("#error_form_qty_"+id).hide();
      $.ajax({
            url : "<?= base_url() ?>modul/lp_gabungan/lp_gabungan_action.php?act=add_temp_barang_jadi",
            data: { 
              kd_barang   :  $("#kode_input_"+id).val(),
              jumlah : $("#form_qty_"+id).val(),
              no : id
            },
            type : 'POST',
            dataType : 'JSON',
     ...
     $.ajax({
          url: "<?= base_url() ?>modul/lp_gabungan/lp_gabungan_action.php?act=add_detail_bahan_baku",
          data: { 
            id: id,
            kd_barang   :  $("#kode_input_"+id).val(),
            kode : $("#form_kode_"+id).val(),
            jumlah : $("#form_qty_"+id).val()
          },
          type : 'POST',
          success: function (data) {
            $("#detail_bahan_baku").html(data);
   ...
   function cek_stok(id,jumlah){
    if ($("#form_kode_"+id).val()=='' && $("#form_qty_"+id).val()=='') {
      $("#error_form_kode_"+id).show();
      $("#error_form_qty_"+id).show();
      $("#form_kode_"+id).focus();
    }else if($("#form_kode_"+id).val()!='' && $("#form_qty_"+id).val()==''){
       // $("#error_form_kode_"+id).show();
      $("#error_form_qty_"+id).show();
      $("#form_qty_"+id).focus();
    }else if($("#form_kode_"+id).val()=='' && $("#form_qty_"+id).val()!=''){
  ...
  function hapus_baris(id) {
      $.ajax({
            url : "<?= base_url() ?>modul/lp_gabungan/lp_gabungan_action.php?act=hapus_temp_barang_jadi",
            data: { 
              kd_barang   :  $("#kode_input_"+id).val(),
              jumlah : $("#form_qty_"+id).val(),
              no : id
     ...
     function add_baris() { 
      ...
      var baris = '<tr id="baris_'+id_baris+'"><td style="text-align: center"><a style="cursor: pointer;" onclick="hapus_baris(\''+id_baris+'\')" ><i class="fa fa-trash-o" style="font-size: 25px;"></i></a> </td><td><input type="text" class="form-control" placeholder="Kode Barang" onclick="cari_kode(\''+id_baris+'\')" id="form_kode_'+id_baris+'" name="kode[]"  > <input type="hidden" id="kode_input_'+id_baris+'" name="kode_input[]" id="kode_input_'+id_baris+'"> <i id="error_form_kode_'+id_baris+'" style="color: red;display:none">Silahkan Pilih Barang Jadi</i> </td><td><input type="text" class="form-control" id="form_unit_'+id_baris+'" name="unit[]" readonly=""></td><td><input type="text"  id="form_qty_'+id_baris+'" class="form-control" name="qty[]"  onkeyup="cek_stok(\''+id_baris+'\',this.value)" required><i id="error_form_qty_'+id_baris+'" style="color: red;display: none">Silahkan input Qty</i><i id="error_stock_'+id_baris+'" style="color: red"></i></td><td><input type="text" class="form-control" name="ket[]" id="form_ket_'+id_baris+'"></td><td><a onclick="add_bahan_baku('+id_baris+')" class="btn btn-primary"><i class="fa fa-list-ol"></i> Bahan Baku/Scrap</a></td></tr>';
