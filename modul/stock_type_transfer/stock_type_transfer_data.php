...
function sttd_h($value) { return htmlspecialchars((string)$value, ENT_QUOTES, 'UTF-8'); }
function sttd_status($status) {
  ...
  return '<span class="label label-default">'.sttd_h($status).'</span>';
}
...
if (!empty($_POST['source_stock_type'])) {
  $where .= " AND h.source_stock_type=? ";
  $params[] = strtoupper(trim($_POST['source_stock_type']));
}
if (!empty($_POST['destination_stock_type'])) {
  $where .= " AND h.destination_stock_type=? ";
  $params[] = strtoupper(trim($_POST['destination_stock_type']));
}
if (!empty($_POST['keyword'])) {
  $kw = '%'.trim($_POST['keyword']).'%';
  $where .= " AND (
    h.transfer_no LIKE ? OR h.reference_no LIKE ? OR h.reason_code LIKE ? OR h.reason_text LIKE ?
    OR EXISTS (SELECT 1 FROM erp_stock_type_transfer_detail d WHERE d.transfer_id=h.id AND (d.material_code LIKE ? OR d.material_name LIKE ?))
    OR EXISTS (SELECT 1 FROM erp_stock_type_transfer_trace t WHERE t.transfer_id=h.id AND (t.no_aju LIKE ? OR t.no_dokpab LIKE ? OR t.no_bpb LIKE ?))
  ) ";
...
$query = $datatable->get_custom(
  "SELECT h.*,
   ...
   LEFT JOIN (
     ...
     FROM erp_stock_type_transfer_trace
     GROUP BY transfer_id
...
foreach ($query as $row) {
  $actions = '<div class="stt-action-buttons">';
  $actions .= '<button type="button" class="btn btn-info btn-xs btn-detail-stt" data-id="'.intval($row->id).'" title="'.wh_h(wh_t('common_detail', 'Detail')).'"><i class="fa fa-eye"></i> <span class="badge">'.intval($row->item_count).'</span></button>';
  if ($row->status === 'POSTED') {
    $actions .= ' <button type="button" class="btn btn-warning btn-xs btn-reversal-stt" data-id="'.intval($row->id).'" data-no="'.sttd_h($row->transfer_no).'" title="'.wh_h(wh_t('warehouse_reversal', 'Reversal')).' "><i class="fa fa-undo"></i></button>';
  }
  $actions .= '</div>';
  $doc = '<strong>'.sttd_h($row->transfer_no).'</strong><br><small>MvT '.sttd_h($row->movement_type).'</small>';
  $source = '<strong>'.sttd_h(trim($row->source_plant_code.' / '.$row->source_storage_code, ' /')).'</strong><br><small>'.sttd_h(trim($row->source_storage_name.' / '.$row->source_bin_code, ' /')).'</small><br><span class="label label-default">'.sttd_h($row->source_stock_type).'</span>';
  $dest = '<strong>'.sttd_h(trim($row->destination_plant_code.' / '.$row->destination_storage_code, ' /')).'</strong><br><small>'.sttd_h(trim($row->destination_storage_name.' / '.$row->destination_bin_code, ' /')).'</small><br><span class="label label-primary">'.sttd_h($row->destination_stock_type).'</span>';
  $reason = '<strong>'.sttd_h($row->reason_code ?: '-').'</strong><br><small>'.sttd_h($row->reason_text ?: '-').'</small>';

  $result = array();
  $result[] = $datatable->number($i);
  $result[] = $actions;
  $result[] = $doc;
  $result[] = sttd_h($row->posting_date);
  $result[] = $source;
  $result[] = $dest;
  $result[] = $reason;
  ...
  $result[] = sttd_status($row->status);
  $result[] = sttd_h($row->created_by);
  $data[] = $result;
  $i++;
}
