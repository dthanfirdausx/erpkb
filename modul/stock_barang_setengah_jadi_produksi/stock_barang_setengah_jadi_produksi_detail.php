...
$summary = $db->fetch(
  "SELECT sl.kode AS kd_barang,COALESCE(b.nm_barang,sl.kode) AS nm_barang,COALESCE(b.satuan,'') AS satuan,
   ...
   WHERE sl.ref_table='erp_gr_production' AND sl.kode=? AND sl.kode LIKE 'BSJ%'
...
$traces = $db->query(
  "SELECT gt.*,gp.gr_no,gp.no_production_order
   ...
   WHERE sl.kode=? AND sl.ref_table='erp_gr_production' AND sl.kode LIKE 'BSJ%'
   ORDER BY gp.posting_date DESC,gt.raw_material_code,gt.no_aju,gt.no_dokpab,gt.id",
  array($kode)
);
?>
<section class="content-header">
  <h1>Detail Stock Barang Setengah Jadi Produksi</h1>
  <ol class="breadcrumb">
...
<section class="content">
  <?php if (!$summary) { ?>
    <div class="alert alert-warning">Stock barang setengah jadi produksi tidak ditemukan dari GR Production.</div>
    <a href="<?=base_index();?>stock-barang-setengah-jadi-produksi" class="btn btn-success"><i class="fa fa-step-backward"></i> <?=$lang["back_button"];?></a>
  <?php } else { ?>
