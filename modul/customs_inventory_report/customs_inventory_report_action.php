...
if ($act === 'detail') {
  session_check_json();
  $layers = cir_load_layers($db, cir_detail_input());
  $totalMasuk=0; $totalUsed=0; $totalSisa=0; $rowCount=0;
  ?>
  <div class="alert alert-info"><strong><?=customs_h('customs_inventory_detail','Detail Customs Inventory.');?></strong> <?=customs_h('customs_inventory_detail_desc','Menampilkan layer/lot pembentuk saldo dokumen pabean ini, termasuk BPB, No Aju, No Pendaftaran, lokasi, dan referensi transaksi. Jika batch fisik belum tersedia, Layer ID menjadi trace key stok.');?></div>
  <div class="table-responsive"><table class="table table-bordered table-condensed cir-detail-table">
    <thead><tr class="bg-gray"><th><?=customs_h('layer_batch','Layer / Batch');?></th><th><?=customs_h('material','Material');?></th><th><?=wh_h(wh_t('warehouse_location', 'Location'));?></th><th><?=customs_h('receipt_date','Receipt Date');?></th><th><?=customs_h('bpb_no','No BPB');?></th><th><?=customs_h('aju_no_short','No Aju');?></th><th><?=customs_h('bc_document','Dokumen BC');?></th><th class="text-right"><?=customs_h('incoming_qty','Qty Masuk');?></th><th class="text-right"><?=customs_h('used_qty','Qty Used');?></th><th class="text-right"><?=customs_h('remaining_qty','Qty Sisa');?></th><th><?=customs_h('reference','Ref');?></th></tr></thead><tbody>
    ...
    <tr><td><strong>Layer #<?=intval($row->id);?></strong><br><small><?=customs_h('trace_key','Trace key');?></small></td><td><strong><?=cir_h($row->kode);?></strong><br><small><?=cir_h($row->nm_barang);?></small></td><td><?=cir_h($location ?: '-');?><br><small><?=cir_h(cir_stock_type_label($row->stock_type));?></small></td><td><?=cir_h($row->tgl_masuk ?: substr((string)$row->created_at,0,10));?></td><td><?=cir_h($row->no_bpb);?></td><td><?=cir_h($row->no_aju);?></td><td><?=cir_h($bc);?></td><td class="text-right"><?=number_format((float)$row->qty_masuk,5,',','.');?></td><td class="text-right"><?=number_format((float)$row->qty_used,5,',','.');?></td><td class="text-right"><strong><?=number_format((float)$row->qty_sisa,5,',','.');?></strong></td><td><small><?=cir_h(trim($row->ref_table.' #'.$row->ref_id,' #'));?></small></td></tr>
    <?php } ?>
...
if ($act === 'excel') {
  ...
  $input = array('tgl_awal'=>cir_input('tgl_awal'),'tgl_akhir'=>cir_input('tgl_akhir'),'material_code'=>cir_input('material_code'),'plant_id'=>cir_input('plant_id'),'storage_location_id'=>cir_input('storage_location_id'),'storage_bin_id'=>cir_input('storage_bin_id'),'stock_type'=>cir_input('stock_type'),'jenis_dokpab'=>cir_input('jenis_dokpab'),'no_aju'=>cir_input('no_aju'),'no_dokpab'=>cir_input('no_dokpab'),'open_only'=>cir_input('open_only','Y'),'keyword'=>cir_input('keyword'));
  $groups = cir_group_layers(cir_load_layers($db,$input));
  $excel = new PHPExcel(); $sheet = $excel->setActiveSheetIndex(0); $sheet->setTitle(erp_export_sheet_title('Customs Inventory Trace'));
  ...
  $tmp=erpkb_excel_temp_file('customs_inventory_');PHPExcel_IOFactory::createWriter($excel,'Excel2007')->save($tmp);$size=@filesize($tmp);$signature=@file_get_contents($tmp,false,null,0,2);
  ...
  while(ob_get_level()>$initialOutputBufferLevel)ob_end_clean();header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');header('Content-Disposition: attachment; filename="customs_inventory_report_'.date('Ymd_His').'.xlsx"');header('Content-Length: '.$size);header('Cache-Control: max-age=0');header('Pragma: public');readfile($tmp);@unlink($tmp);exit;
}
