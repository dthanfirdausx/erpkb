...
$columns = array(
  ...
  $query = $datatable->get_custom("select id,id_barang, vtotalstockprodbb.kd_barang,vtotalstockprodbb.nm_barang,vtotalstockprodbb.Stock,vtotalstockprodbb.satuan,vtotalstockprodbb.nm_kategori,vtotalstockprodbb.kd_barang from v_stock_produksi vtotalstockprodbb where 1=1 and Stock>0 $wh",$columns);
  ...
  foreach ($query as $value) {
    ...
    // $ResultData[] = "<button class='btn btn-primary' style='font-size:12px' id='btn_$value->id' onclick='sinkron_stock(\"$value->id_barang\",\"3\",\"$value->id\")'><i class='fa fa-gear'></i> Sinkron Stock </button>";
