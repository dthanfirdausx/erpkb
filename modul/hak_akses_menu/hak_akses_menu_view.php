...
foreach ($db->query("select sys_group_users.id, sys_group_users.level_name 
  from sys_users right join sys_group_users 
...
<table id="dtb" class="table table-bordered table-condensed table-hover table-striped">
                      ...
                      <tbody>
                        <?php 
      $dtb=$db->query("select sys_menu.type_menu,sys_menu_role.read_act,sys_menu_role.insert_act,sys_menu_role.update_act,sys_menu_role.delete_act, import_act,sys_menu.page_name,sys_menu.urutan_menu,sys_group_users.level_name,sys_menu_role.id from sys_menu_role inner join sys_menu on sys_menu_role.id_menu=sys_menu.id inner join sys_group_users on sys_menu_role.group_level=sys_group_users.level where sys_group_users.id=? and sys_menu.tampil='Y'  order by urutan_menu asc",array('sys_group_users.id'=>$_GET
        ['user']));
      $i=1;
      foreach ($dtb as $isi) {
        ?><tr id="line_<?=$isi->id;?>">
