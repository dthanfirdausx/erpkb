...
if ($act === 'detail') {
  ...
  ?>
  <style>.tpr-detail-table th{width:190px;background:#f8fafc}.tpr-detail-table th,.tpr-detail-table td{font-size:12px}.tpr-detail-head{border:1px solid #e5e7eb;background:#f8fafc;border-radius:12px;padding:14px;margin-bottom:14px}</style>
  <div class="tpr-detail-head">
    <div class="row">
      <div class="col-sm-8"><h3 style="margin-top:0"><?=tpr_h($doc ?: '-');?> <small>Item <?=intval($row->id_detail);?></small></h3><p>Movement Type <?=tpr_h($row->move_code);?> - <?=tpr_h(tpr_movement_label($row->move_code,$row->ref_type,$direction));?> | <span class="label label-<?=$direction==='OUT'?'danger':'success';?>"><?=tpr_h($direction);?></span></p></div>
      <div class="col-sm-4 text-right"><h3 style="margin-top:0"><?=number_format(abs((float)$row->qty),5,',','.');?> <?=tpr_h($row->uom ?: $row->material_uom);?></h3><p>Amount <?=number_format((float)$row->amount,2,',','.');?></p></div>
    </div>
  </div>
  <div class="row">
    <div class="col-md-6"><h4><?=wh_h(wh_t('warehouse_document', 'Document'));?></h4><table class="table table-bordered table-condensed tpr-detail-table">
      <tr><th>Material Document</th><td><?=tpr_h($doc);?></td></tr>
      <tr><th><?=wh_h(wh_t('warehouse_posting_date', 'Posting Date'));?></th><td><?=tpr_h($row->posting_date);?></td></tr>
      <tr><th>Document Date</th><td><?=tpr_h($row->document_date);?></td></tr>
      <tr><th><?=wh_h(wh_t('warehouse_reference_type', 'Reference Type'));?></th><td><?=tpr_h($row->ref_type);?></td></tr>
      <tr><th>Created By</th><td><?=tpr_h($row->created_by ?: $row->user);?></td></tr>
      <tr><th>Remark / Reason</th><td><?=tpr_h(trim((string)$row->remark.' '.$row->reason));?></td></tr>
    </table></div>
    <div class="col-md-6"><h4>Material & Quantity</h4><table class="table table-bordered table-condensed tpr-detail-table">
      <tr><th><?=wh_h(wh_t('warehouse_material', 'Material'));?></th><td><?=tpr_h(trim((string)($row->destination_material_code ?: $row->kd_barang).' - '.(string)$row->nm_barang,' -'));?></td></tr>
      <tr><th><?=wh_h(wh_t('warehouse_movement', 'Movement'));?></th><td><?=tpr_h($row->move_code.' - '.tpr_movement_label($row->move_code,$row->ref_type,$direction));?></td></tr>
      <tr><th><?=wh_h(wh_t('warehouse_direction', 'Direction'));?></th><td><?=tpr_h($direction);?></td></tr>
      <tr><th><?=wh_h(wh_t('warehouse_qty', 'Qty'));?></th><td><?=number_format(abs((float)$row->qty),5,',','.');?> <?=tpr_h($row->uom ?: $row->material_uom);?></td></tr>
  ...
  <div class="row">
    <div class="col-md-6"><h4><?=wh_h(wh_t('warehouse_location', 'Location'));?></h4><table class="table table-bordered table-condensed tpr-detail-table">
      <tr><th><?=wh_h(wh_t('common_plant', 'Plant'));?></th><td><?=tpr_h(trim((string)$row->plant_code.' - '.(string)$row->plant_name,' -'));?></td></tr>
      <tr><th><?=wh_h(wh_t('warehouse_storage_location', 'Storage Location'));?></th><td><?=tpr_h(trim((string)$row->storage_code.' - '.(string)$row->storage_name,' -'));?></td></tr>
      <tr><th><?=wh_h(wh_t('warehouse_storage_bin', 'Storage Bin'));?></th><td><?=tpr_h(trim((string)$row->bin_code.' - '.(string)$row->bin_name,' -'));?></td></tr>
      <tr><th><?=wh_h(wh_t('warehouse_stock_type', 'Stock Type'));?></th><td><?=tpr_h(tpr_stock_type_label($stockType));?></td></tr>
      <tr><th>Legacy Location</th><td><?=tpr_h($row->lokasi);?></td></tr>
    </table></div>
    <div class="col-md-6"><h4>Customs & Reference</h4><table class="table table-bordered table-condensed tpr-detail-table">
      <tr><th>No Aju</th><td><?=tpr_h($row->no_aju ?: $row->header_no_aju);?></td></tr>
      <tr><th>Dokumen BC</th><td><?=tpr_h(trim((string)$row->jenis_dokpab.' '.(string)($row->no_dokpab ?: $row->header_no_dokpab)));?></td></tr>
      <tr><th>Tgl Dokumen BC</th><td><?=tpr_h($row->tgl_dokpab);?></td></tr>
      <tr><th>PO</th><td><?=tpr_h($row->purchase_order_no);?></td></tr>
      <tr><th>Vendor</th><td><?=tpr_h($row->vendor_name);?></td></tr>
      <tr><th>Reversal Ref</th><td><?=tpr_h(trim((string)$row->original_doc.' '.(string)$row->original_mvt.' '.(string)$row->original_posting_date));?></td></tr>
    </table></div>
  </div>
  <?php
  exit;
...
if ($act === 'excel') {
  ...
  $rows = tpr_load_rows($db, $input);
  $summary = tpr_summary($db, $input);
  $from = tpr_valid_date($input['tgl_awal'], date('Y-m-01'));
  $to = tpr_valid_date($input['tgl_akhir'], date('Y-m-d'));

  $excel = new PHPExcel();
  $sheet = $excel->setActiveSheetIndex(0);
  $sheet->setTitle(erp_export_sheet_title('Transfer Posting'));
  ...
  foreach ($rows as $row) {
    ...
    $values = array($n++,$doc,$row->id_detail,$row->posting_date,$row->document_date,$row->move_code,tpr_movement_label($row->move_code,$row->ref_type,$direction),$direction,$row->ref_type,$row->material_code,$row->nm_barang,$row->plant_code,$row->storage_code,$row->bin_code,tpr_stock_type_label($row->stock_type_label),$qtyIn,$qtyOut,$row->uom ?: $row->satuan,(float)$row->price,(float)$row->amount,$row->no_aju ?: $row->header_no_aju,$row->no_dokpab ?: $row->header_no_dokpab,$row->purchase_order_no,$row->vendor_name,$row->username,$row->remark ?: $row->reason);
  ...
  erpkb_excel_apply_standard_style($excel,array(
    'sheet'=>$sheet,
    'title'=>erp_export_title('TRANSFER POSTING REPORT - SAP MM'),
    'header_row'=>4,
    'first_data_row'=>5,
  ...
  $tmp = erpkb_excel_temp_file('transfer_posting_');
  ...
  header('Content-Disposition: attachment; filename="transfer_posting_report_'.$from.'_sd_'.$to.'.xlsx"');
