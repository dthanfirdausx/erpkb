...
?>
 ...
 <div class="alert alert-warning fade in error_data_delete" style="display:none">
    ...
    <div class="modal" id="modal_bc_keluar" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"> <div class="modal-dialog modal-lg"> <div class="modal-content"><div class="modal-header"> <button type="button" class="close" data-dismiss="modal" aria-label="<?=erp_attr('common_close','Close');?>"><span aria-hidden="true">×</span></button> <h4 class="modal-title"><?=erp_h('add_button','Add New');?> <?=erp_h('master_outbound_customs_type','Outbound Customs Type');?></h4> </div> <div class="modal-body" id="isi_bc_keluar"> </div> </div><!-- /.modal-content --> </div><!-- /.modal-dialog --> </div>
      ...
      $("#add_bc_keluar").click(function() {
          $.ajax({
              url : "<?=base_admin();?>modul/bc_keluar/bc_keluar_add.php",
              type : "GET",
              success: function(data) {
                  $("#isi_bc_keluar").html(data);
              }
          });

      $('#modal_bc_keluar').modal({ keyboard: false,backdrop:'static',show:true });
    ...
    $(".table").on('click','.edit_data',function(event) {
        ...
        $.ajax({
            url : "<?=base_admin();?>modul/bc_keluar/bc_keluar_edit.php",
            type : "post",
            data : {id_data:id},
            success: function(data) {
                $("#isi_bc_keluar").html(data);
    ...
    $('#modal_bc_keluar').modal({ keyboard: false,backdrop:'static' });
      ...
      var dtb_bc_keluar = $("#dtb_bc_keluar").DataTable({
           ...
           'columnDefs': [ {
            ...
            'ajax':{
              url :'<?=base_admin();?>modul/bc_keluar/bc_keluar_data.php',
  ...
  $('#dtb_bc_keluar').on('draw.dt', function() {
          init_selected()
      });
  ...
  $(document).on('click', '#dtb_bc_keluar tbody tr td', function(event) {
  ...
  function check_selected() {
      var table_select = $('#dtb_bc_keluar tbody tr.selected');
  ...
  function select_deselect(type) {
      if (type == 'select') {
          $('#dtb_bc_keluar tbody tr').addClass('DTTT_selected selected')
      } else {
          $('#dtb_bc_keluar tbody tr').removeClass('DTTT_selected selected')
      }
      init_selected()
  }
...
/* Add a click handler for the delete row */
  $('#bulk_delete').click( function() {
    var anSelected = fnGetSelected( dtb_bc_keluar );
    ...
    $('#ucing').modal({ keyboard: false }).one('click', '#delete', function (e) {
        ...
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: '<?=base_admin();?>modul/bc_keluar/bc_keluar_action.php?act=del_massal',
