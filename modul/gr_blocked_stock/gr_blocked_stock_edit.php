<!-- Content Header (Page header) -->
              <section class="content-header">
                  <h1>GR Blocked Stock </h1>
                    <ol class="breadcrumb">
                        <li>
                        <a href="<?=base_index();?>"><i class="fa fa-dashboard"></i> Home</a>
                        </li>
                        <li>
                        <a href="<?=base_index();?>pemasukan-">GR Blocked Stock </a>
                        </li>
                        <li class="active">Edit GR Blocked Stock </li>
                    </ol>
              </section>
                  ...
                  <div class="col-lg-12">
                      <div class="box box-solid box-primary">
                          <div class="box-header">
                              <h3 class="box-title">Edit GR Blocked Stock </h3>
                      ...
                      <div class="box-body">
                       <div class="alert alert-danger error_data" style="display:none">
                        ...
                        <span class="isi_warning"></span>
                          ...
                          <form id="edit_pemasukan_" method="post" class="form-horizontal" action="<?=base_admin();?>modul/gr_blocked_stock/gr_blocked_stock_action.php?act=in">
                            
              <div class="form-group">  
                <label for="No BPB" class="control-label col-lg-2">No Ref Lama </label>
                <div class="col-lg-10">
                  <input type="text" name="no_bpb_lama" value="<?=$data_edit->no_bpb;?>" class="form-control" readonly >
          ...
          <div class="form-group">
              ...
              <select  id="nopo" name="nopo" data-placeholder="Pilih No PO..." class="form-control chzn-select" tabindex="2" onchange="pilih_po(this.value)"  >
               <option value=""></option>
               <?php foreach ($db->query("select `po`.`nopo` AS `nopo`,`p`.`nopo` AS `nopo_pemasukan` from (`po` left join `pemasukan` `p` on(`p`.`nopo` = `po`.`nopo`)) ") as $isi) { 
...
<div class="form-group">
              ...
              <div class="col-lg-3">
                <div class="input-group date" id="tgl2">
                    <input type="text" class="form-control" value="<?=$data_edit->tgl_invoice;?>" name="tgl_invoice" autocomplete="off"  />
                    <span class="input-group-addon">
                        <span class="glyphicon glyphicon-calendar"></span>
                    </span>
                </div>
          ...
          ?>
            <div class="form-group">
                ...
                <div class="col-lg-10">
                    <input type="hidden" name="jenis_dokpab_lama" id="jenis_dokpab_lama" value="<?= $data_edit->jenis_dokpab ?>" >
          ...
          <div class="form-group">
              ...
              <div class="form-group">
                <!--  <label for="Kurs" class="control-label col-lg-2"> </label> -->
                 ...
                 <thead>
                   <tr>
                     ...
                     <th><?=wh_h(wh_t('warehouse_qty', 'Qty'));?></th>
                     <th>Harga</th>
                     <th>Nilai</th>
                     <th>Berat</th>
                     <th>Lokasi</th>
                 ...
                 $q = $db->query("select d.*,ba.nm_barang
                              from pemasukan_detail d join barang ba on ba.kd_barang=d.kode where d.no_bpb='$data_edit->no_bpb'  "); 
                 $no=1;
                 foreach ($q as $k) {
                  ?>
                  <tr id="baris_<?= $no ?>">
                     <td style="text-align: center"><a style="cursor: pointer;" onclick="hapus_baris('<?= $no ?>','<?= $k->id ?>')" ><i class="fa fa-trash-o" style="font-size: 25px;"></i></a> </td>
                     <td><input type="text" id="form_kode_<?= $no ?>" value="<?= $k->kode." , ".$k->nm_barang ?>" placeholder="Kode Barang" onclick="cari_kode('<?= $no ?>')" class="form-control" name="kode[]" style="width: 300px" >
                      <input type="hidden" name="kode_input[]" value="<?= $k->kode ?>" id="kode_input_<?= $no ?>"> 
                     </td>   
                     <td><input type="text" id="form_unit_<?= $no ?>" value="<?= $k->unit ?>"  class="form-control" name="unit[]"  readonly=""></td> 
               ...
               <input type="hidden" id="jml" value="<?= ($no-1) ?>">
                            ...
                            <input type="hidden" name="id" id="id" value="<?=$data_edit->no_bpb;?>">
                            <div class="form-group">
...
<script type="text/javascript">
    function hapus_baris(id) {
      ...
      if (result.isConfirmed) {
        $.ajax({
                   url: "<?= base_url() ?>modul/gr_blocked_stock/gr_blocked_stock_action.php?act=hapus_detail",
                   data: {
                        kode   : $("#kode_input_"+id).val(),  
                        no_bpb : $("#id").val(),
                        jumlah : $("#form_qty_"+id).val(), 
                        jenis_dokpab : $("#jenis_dokpab_lama").val()
                    },
                    type : 'POST', 
      ...
      function cari_kode(id) {   
                      ...
                      $('#form_kode_'+id).autocomplete({
                        source: function (request, response) {
                          $.ajax({
                            url: "<?= base_url() ?>modul/gr_blocked_stock/gr_blocked_stock_action.php?act=cari_kode",
                        ...
                        select: function (event, ui) {
                            ...
                            $.ajax({
                              ...
                              url : "<?= base_url() ?>modul/gr_blocked_stock/gr_blocked_stock_action.php?act=get_unit",
                              success:function(data){
            ...
            $(form).ajaxSubmit({
                ...
                success: function(responseText) {
                  ...
                  $("#jenisbcmasuk_jenis_dokumen").change(function(){
                        ...
                        $.ajax({
                        type : "post",
                        url : "<?=base_admin();?>modul/gr_blocked_stock/get_kd_catdet.php",
