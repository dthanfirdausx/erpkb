...
<section class="content-header">
  <h1>GR Blocked Stock <small>SAP MM Movement Type 103</small></h1>
  <ol class="breadcrumb"><li><a href="<?=base_index();?>"><i class="fa fa-dashboard"></i> Home</a></li><li><a href="<?=base_index();?>gr-blocked-stock">Goods Receipt</a></li><li class="active">Create</li></ol>
</section>
<section class="content">
<form id="input_gr_blocked_stock" method="post" action="<?=base_admin();?>modul/gr_blocked_stock/gr_blocked_stock_action.php?act=in">
  ...
  <div class="box box-primary gr-section">
    ...
    <div class="box-body"><div class="row">
      ...
      <div class="col-md-3 form-group"><label><?=wh_h(wh_t('warehouse_movement_type', 'Movement Type'));?></label><input class="form-control" value="103 - GR Blocked Stock" readonly><input type="hidden" name="move_code" value="103"></div>
      <div class="col-md-3 form-group"><label><?=wh_h(wh_t('warehouse_stock_type', 'Stock Type'));?></label><input class="form-control" value="Blocked Stock" readonly><input type="hidden" name="stock_type" value="BLOCKED"></div>
    </div></div>
  </div>
  ...
  <div class="box box-success gr-section">
    ...
    <div class="box-body"><p class="help-customs">Untuk GR Blocked Stock, material tidak ditambah manual di form ini. Tambahkan atau revisi item di Purchase Order terlebih dahulu, lalu pilih ulang PO di form GR.</p><div class="gr-items-scroll"><table class="table table-bordered gr-items"><thead><tr>
      <th></th><th>PO Item</th><th><?=wh_h(wh_t('warehouse_material', 'Material'));?></th><th><?=wh_h(wh_t('warehouse_uom', 'UOM'));?></th><th>Open Qty</th><th class="required-label">GR Qty</th><th>Batch/Lot</th><th><?=wh_h(wh_t('warehouse_storage_bin', 'Storage Bin'));?></th><th><?=wh_h(wh_t('warehouse_amount', 'Amount'));?></th><th><?=wh_h(wh_t('common_detail', 'Detail'));?></th>
    </tr></thead><tbody id="isi_tabel"></tbody></table></div><input type="hidden" id="jml" value="0"></div>
  </div>
  <div class="text-right"><span id="gr_submit_help" class="gr-submit-help">Lengkapi semua field mandatory untuk posting GR Blocked Stock.</span><a href="<?=base_index();?>gr-blocked-stock" class="btn btn-default"><i class="fa fa-arrow-left"></i> Kembali</a> <button type="submit" id="btn_post_gr" class="btn btn-primary" disabled><i class="fa fa-save"></i> <?=wh_h(wh_t('common_post', 'Post'));?> GR Blocked Stock</button></div>
</form>
</section>
...
function updateSubmitState(){var valid=true;$('#input_gr_blocked_stock [required]').each(function(){var ok=isRequiredFilled(this);$(this).toggleClass('gr-required-missing',!ok);if(!ok)valid=false;});if($('#isi_tabel .gr-main-row').length===0)valid=false;$('#btn_post_gr').prop('disabled',!valid);$('#gr_submit_help').toggle(!valid);return valid;}
function openFirstInvalidDetail(){var first=$('#input_gr_blocked_stock .gr-required-missing').first();if(first.length){var detail=first.closest('.gr-detail-row');if(detail.length)detail.show();$('html,body').animate({scrollTop:first.offset().top-120},300);}}
function loadPo(noPo){if(!noPo){$('#isi_tabel').empty();updateSubmitState();return;}$.post('<?=base_url();?>modul/gr_blocked_stock/gr_blocked_stock_action.php?act=get_po_form',{no_po:noPo},function(res){if(!res||res.status!=='good')return;$('#pemasok').val(res.header.seller_code).trigger('chosen:updated');$('#valuta').val(res.header.currency).trigger('chosen:updated');$('#isi_tabel').empty();$('#jml').val(0);$.each(res.items,function(_,item){addGrRow(item);});updateSubmitState();},'json');}
function showGrError(message){$('.isi_warning').text(message||'Terjadi error saat posting GR Blocked Stock.');$('.error_data').show();$('html,body').animate({scrollTop:$('.error_data').offset().top-90},300);}
...
$(function(){
  ...
  $('#input_gr_blocked_stock').on('input change','input,select,textarea',updateSubmitState);
  ...
  $('#jenisbcmasuk_jenis_dokumen').change(function(){$.post('<?=base_admin();?>modul/gr_blocked_stock/get_kd_catdet.php',{jenis_dokumen:this.value},function(data){$('#detail_catatan_kd_catdet').html(data).trigger('chosen:updated');updateSubmitState();});});
  $('#input_gr_blocked_stock').on('submit',function(e){
    e.preventDefault();
    if(!updateSubmitState()){showGrError('Lengkapi semua field mandatory sebelum posting GR Blocked Stock.');openFirstInvalidDetail();return false;}
    var invalid=false;
    ...
    $.ajax({
      ...
      success:function(response){
        ...
        if(result&&result.status==='good'){
          window.location='<?=base_index();?>gr-blocked-stock';
          return;
        }
        showGrError(result&&result.error_message?result.error_message:'Posting GR Blocked Stock gagal.');
      ...
      error:function(xhr){
        ...
        showGrError(result&&result.error_message?result.error_message:'Server error saat posting GR Blocked Stock.');
        button.prop('disabled',false).html(button.data('original-text'));
      }
    });
    return false;
