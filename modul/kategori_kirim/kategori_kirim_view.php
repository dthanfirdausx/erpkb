<!-- Content Header (Page header) -->
 ...
 <div class="alert alert-warning fade in error_data_delete" style="display:none">
    ...
    <div class="modal" id="modal_kategori_kirim" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"> <div class="modal-dialog modal-lg"> <div class="modal-content"><div class="modal-header"> <button type="button" class="close" data-dismiss="modal" aria-label="<?=erp_attr('common_close','Close');?>"><span aria-hidden="true">×</span></button> <h4 class="modal-title"><?=erp_h('add_button','Add New');?> <?=erp_h('master_shipping_category','Shipping Category');?></h4> </div> <div class="modal-body" id="isi_kategori_kirim"> </div> </div><!-- /.modal-content --> </div><!-- /.modal-dialog --> </div>
      ...
      $("#add_kategori_kirim").click(function() {
          $.ajax({
              url : "<?=base_admin();?>modul/kategori_kirim/kategori_kirim_add.php",
              type : "GET",
              success: function(data) {
                  $("#isi_kategori_kirim").html(data);
              }
          });

      $('#modal_kategori_kirim').modal({ keyboard: false,backdrop:'static',show:true });
    ...
    $(".table").on('click','.edit_data',function(event) {
        ...
        $.ajax({
            url : "<?=base_admin();?>modul/kategori_kirim/kategori_kirim_edit.php",
            type : "post",
            data : {id_data:id},
            success: function(data) {
                $("#isi_kategori_kirim").html(data);
    ...
    $('#modal_kategori_kirim').modal({ keyboard: false,backdrop:'static' });
      ...
      var dtb_kategori_kirim = $("#dtb_kategori_kirim").DataTable({
           ...
           'columnDefs': [ {
            ...
            'ajax':{
              url :'<?=base_admin();?>modul/kategori_kirim/kategori_kirim_data.php',
  ...
  $('#dtb_kategori_kirim').on('draw.dt', function() {
          init_selected()
      });
  ...
  $(document).on('click', '#dtb_kategori_kirim tbody tr td', function(event) {
  ...
  function check_selected() {
      var table_select = $('#dtb_kategori_kirim tbody tr.selected');
  ...
  function select_deselect(type) {
      if (type == 'select') {
          $('#dtb_kategori_kirim tbody tr').addClass('DTTT_selected selected')
      } else {
          $('#dtb_kategori_kirim tbody tr').removeClass('DTTT_selected selected')
      }
      init_selected()
  }
...
/* Add a click handler for the delete row */
  $('#bulk_delete').click( function() {
    var anSelected = fnGetSelected( dtb_kategori_kirim );
    ...
    $('#ucing').modal({ keyboard: false }).one('click', '#delete', function (e) {
        ...
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: '<?=base_admin();?>modul/kategori_kirim/kategori_kirim_action.php?act=del_massal',
