...
$title = finrep_t('finance_report_cash_flow_indirect_multi_period', 'Arus Kas Multi Period (Tak Langsung)');
?>
<style>
...
<section class="content">
  <div class="box box-primary">
    <div class="box-header with-border">
      <h3 class="box-title"><?=finrep_h(finrep_t('finance_indirect_cash_flow_multi_period_statement', 'Laporan Arus Kas Tak Langsung Multi Period'));?></h3>
      <div class="box-tools">
...
$(function(){
  ...
  function loadReport(){
    ...
    $.ajax({url:'<?=base_admin();?>modul/finance_report/finance_report_arus_kas_multi_period_tak_langsung_action.php?act=filter',type:'POST',dataType:'json',data:qs()}).done(function(res){
  ...
  $('#btn_excel').on('click',function(){ window.location='<?=base_admin();?>modul/finance_report/finance_report_arus_kas_multi_period_tak_langsung_action.php?act=excel&'+qs(); });
  $('#btn_print').on('click',function(){ window.open('<?=base_admin();?>modul/finance_report/finance_report_arus_kas_multi_period_tak_langsung_action.php?act=print&'+qs(), '_blank'); });
  loadReport();
});
</script>
