...
// update status jadi reversal (9)
$db->query("UPDATE brgjadi SET status='9' WHERE id_produksi='$id'");

// 🔥 ambil semua transaksi
$q = $db->query("
    SELECT * FROM detail_transaksi 
    WHERE no_ref IN (
        SELECT no_bpb FROM brgjadi WHERE id_produksi='$id'
    )
");
...
foreach ($kode_jadi as $i => $kode){
    ...
    $qty = (float)$qty_jadi[$i];
    if ($kode == '' || $qty <= 0) continue;

    $total_qty_jadi += $qty;

    // 🔥 ledger hasil (TAMBAH no_bpb)
    $db->insert("detail_transaksi", [
        'kd_barang'    => $kode,
        'qty'          => $qty,
        'posisi'       => 'PRODUKSI',
        'move_code'    => '101',
        'no_ref'       => $no_bpb,
        'no_bpb'       => $no_bpb, // 🔥 penting
        'remark'       => 'Hasil Produksi',
    ...
    // 🔥 stock_layer hasil (TAMBAH no_bpb)
