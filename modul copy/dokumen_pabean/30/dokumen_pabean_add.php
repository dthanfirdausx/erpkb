...
<script type="text/javascript">
  ...
  function save_data_hitung(val,kolom,id,tabel,kol_id){
      $.ajax({
       url : "<?= base_url() ?>modul/dokumen_pabean/dokumen_pabean_action.php?act=save_data",
       type : "POST",
       data : {
      ...
      // dataTye : 'JSON',
       success : function(data){ 
          ...
          var fob2          = parseFloat(fob.replace(",", "")); 
          var satuan        = parseFloat($("#jumlahSatuan").val());   
          var hargaSatuan   = $("#hargaSatuan").val((fob2/satuan).toFixed(3));
          var hargaSatuan2  = $("#hargaSatuan").val();  
          save_data(hargaSatuan2,'hargaSatuan',$('#id_barang').val(),'ws_barang','idBarang');
       }
    }); 
  }

  function save_data_barang(val,kolom,id,tabel,kol_id){
      $.ajax({
       url : "<?= base_url() ?>modul/dokumen_pabean/dokumen_pabean_action.php?act=save_data",
       type : "POST",
       data : {
      ...
      // dataTye : 'JSON',
       success : function(data){
         $.ajax({
           url : "<?= base_url() ?>modul/dokumen_pabean/dokumen_pabean_action.php?act=get_uraian",
           type : "POST",
           data : {
             id : val
           },
          // dataTye : 'JSON',
           success : function(data2){
            ...
            if (data2!='') {  
              $("#uraian").val(data2); 
              save_data(data2,'uraian',$('#id_barang').val(),'ws_barang','idBarang');
            }
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
          ...
          $('.form-lokasi-periksa').select2();
          $('.form-tujuan').select2(); 
          $('.form-ref-dokumen').select2();   
          $('.form-jenis-exsport').select2();
          $('.form-kategori-exsport').select2();
          $('.form-cara-bayar').select2();
          $('.form-cara-dagang').select2();
          $('.form-komoditi').select2();
          $('.form-curah').select2();
          $('.form-bank').select2(); 
          $('.form-kantor').select2({
                ...
                tags: [],
                ajax: {
                    url: '<?= base_url() ?>modul/dokumen_pabean/dokumen_pabean_action.php?act=get_kantor',
                    dataType: 'json',
                    type: "GET",
                    quietMillis: 50,
                    data: function (term) { 
          ...
          $('.form-penerima').select2({   
                minimumInputLength: 2,
                tags: [],
                ajax: {
                    url: '<?= base_url() ?>modul/dokumen_pabean/dokumen_pabean_action.php?act=get_penerima',
                    dataType: 'json',
                    type: "GET",
                    quietMillis: 50,
                    data: function (term) { 
...
$('.form-pelabuhan-transit').append(newOptionTran).trigger('change');
 ...
 var dataKantor = {
    id: "<?= $data_header->kodeKantorBongkar ?>",
    text: "<?= $data_header->kodeKantorBongkar." - ".$data_header->nama_kantor ?>"
};


var newOptionKantor = new Option(dataKantor.text, dataKantor.id, false, false);
$('.form-kantor').append(newOptionKantor).trigger('change'); 
