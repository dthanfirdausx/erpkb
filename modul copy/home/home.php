...
series:[{name:'Jumlah',data: <?= json_encode($data) ?>}]
});
...
series:[{name:'Jumlah',data: <?= json_encode($data2) ?>}]
});

// PIE
Highcharts.chart('chart_pie', {
chart:{type:'pie'},
series:[{name:'Dokumen',data: <?= json_encode($pie) ?>}]
});
...
series:[{name:'Transaksi',data: <?= json_encode($trend) ?>}]
});
