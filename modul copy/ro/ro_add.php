           ...
           <!--  <select  id="id_bom" onchange="get_detail_bom(this.value)" data-placeholder="Pilih Material ..." class="js-example-basic-multiple" name="id_bom[]" multiple="multiple" style="width: 100%" >
            ...
            <table class="table">
              ...
              <tbody id="barang_pilih">
                <?php
                $q = $db->query("select * from v_ro_barang where barang_temp='1' and user='".$_SESSION['IKB4_status_UserName']."' ");
  foreach ($q as $k) {
    $jml_order = 1;
    if ($k->jml!='') {
      $jml_order = $k->jml;  
                 ...
                 <div class="col-lg-10">
                   <table class="table">
                     ...
                     <tbody id="isi_tabel">
                      <?php
                      $no=1;
                      $qq = $db->query("select b.id,b.kodebj,v.jml from v_ro_barang v join bom b on b.kodebj=v.kd_barang where barang_temp='1' and user='".$_SESSION['IKB4_status_UserName']."' ");
                      foreach ($qq as $kk) {
                        $jml = 1;
                        if ($kk->jml!='') { 
                         $jml = $kk->jml;
